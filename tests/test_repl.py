...
class TestHelperFunctionBuscar:
    """Test the buscar() helper function available in REPL namespace."""

    def test_buscar_is_available_in_namespace(self):
        """buscar function is available in the REPL namespace."""
        repl = SafeREPL()
        result = repl.execute("print(callable(buscar))")

        assert result.success is True
        assert "True" in result.stdout

    def test_buscar_finds_single_occurrence(self):
        """buscar finds a single occurrence of a term."""
        repl = SafeREPL()
        result = repl.execute("""
texto = "O gato sentou no tapete."
resultados = buscar(texto, "gato")
...
""")
    ...
    def test_buscar_finds_multiple_occurrences(self):
        """buscar finds multiple occurrences of a term."""
        repl = SafeREPL()
        result = repl.execute("""
texto = "O gato viu outro gato no telhado."
resultados = buscar(texto, "gato")
print(len(resultados))
""")
    ...
    def test_buscar_returns_empty_for_no_matches(self):
        """buscar returns empty list when term is not found."""
        repl = SafeREPL()
        result = repl.execute("""
texto = "O cachorro latiu."
resultados = buscar(texto, "gato")
print(len(resultados))
""")
    ...
    def test_buscar_is_case_insensitive(self):
        """buscar is case-insensitive."""
        repl = SafeREPL()
        result = repl.execute("""
texto = "O GATO sentou."
resultados = buscar(texto, "gato")
...
""")
    ...
    def test_buscar_returns_position(self):
        """buscar returns the position of each occurrence."""
        repl = SafeREPL()
        result = repl.execute("""
texto = "O gato."
resultados = buscar(texto, "gato")
print(resultados[0]['posicao'])
""")

        assert result.success is True
        assert "2" in result.stdout  # "gato" starts at position 2

    def test_buscar_returns_line_number(self):
        """buscar returns the line number of each occurrence."""
        repl = SafeREPL()
        result = repl.execute("""
texto = "Linha 1\\nO gato na linha 2\\nLinha 3"
resultados = buscar(texto, "gato")
print(resultados[0]['linha'])
""")

        assert result.success is True
        assert "2" in result.stdout

    def test_buscar_returns_context(self):
        """buscar returns context around each occurrence."""
        repl = SafeREPL()
        result = repl.execute("""
texto = "Antes do gato depois."
resultados = buscar(texto, "gato")
print(resultados[0]['contexto'])
""")

        assert result.success is True
        assert "gato" in result.stdout
        assert "Antes" in result.stdout

    def test_buscar_handles_empty_text(self):
        """buscar handles empty text input."""
        repl = SafeREPL()
        result = repl.execute("""
resultados = buscar("", "gato")
print(len(resultados))
""")
    ...
    def test_buscar_handles_empty_term(self):
        """buscar handles empty search term."""
        repl = SafeREPL()
        result = repl.execute("""
...
""")
    ...
    def test_buscar_result_is_list_of_dicts(self):
        """buscar returns a list of dictionaries."""
        repl = SafeREPL()
        result = repl.execute("""
texto = "O gato."
resultados = buscar(texto, "gato")
print(isinstance(resultados, list))
print(isinstance(resultados[0], dict))
""")

        assert result.success is True
        assert result.stdout.count("True") == 2

    def test_buscar_dict_has_required_keys(self):
        """buscar result dicts have posicao, linha, and contexto keys."""
        repl = SafeREPL()
        result = repl.execute("""
texto = "O gato."
resultados = buscar(texto, "gato")
keys = set(resultados[0].keys())
print('posicao' in keys)
print('linha' in keys)
print('contexto' in keys)
""")

        assert result.success is True
        assert result.stdout.count("True") == 3

    def test_buscar_is_not_saved_as_user_variable(self):
        """buscar function is not counted as a user variable."""
        repl = SafeREPL()
        repl.execute("x = 1")

        user_vars = [v.name for v in repl.list_variables()]
        assert 'buscar' not in user_vars
        assert 'x' in user_vars


class TestHelperFunctionContar:
    """Test the contar() helper function available in REPL namespace."""

    def test_contar_is_available_in_namespace(self):
        """contar function is available in the REPL namespace."""
        repl = SafeREPL()
        result = repl.execute("print(callable(contar))")

        assert result.success is True
        assert "True" in result.stdout

    def test_contar_counts_single_occurrence(self):
        """contar counts a single occurrence of a term."""
        repl = SafeREPL()
        result = repl.execute("""
texto = "O gato sentou no tapete."
resultado = contar(texto, "gato")
...
""")
    ...
    def test_contar_counts_multiple_occurrences(self):
        """contar counts multiple occurrences of a term."""
        repl = SafeREPL()
        result = repl.execute("""
texto = "O gato viu outro gato no telhado."
resultado = contar(texto, "gato")
...
""")
    ...
    def test_contar_returns_zero_for_no_matches(self):
        """contar returns zero when term is not found."""
        repl = SafeREPL()
        result = repl.execute("""
texto = "O cachorro latiu."
resultado = contar(texto, "gato")
...
""")
    ...
    def test_contar_is_case_insensitive(self):
        """contar is case-insensitive."""
        repl = SafeREPL()
        result = repl.execute("""
texto = "O GATO sentou. O gato dormiu."
resultado = contar(texto, "gato")
print(resultado['total'])
""")
    ...
    def test_contar_counts_per_line(self):
        """contar returns count per line."""
        repl = SafeREPL()
        result = repl.execute("""
texto = "gato na linha 1\\nLinha 2 sem nada\\ngato gato na linha 3"
resultado = contar(texto, "gato")
print(resultado['por_linha'].get(1, 0))
print(resultado['por_linha'].get(2, 0))
print(resultado['por_linha'].get(3, 0))
""")

        assert result.success is True
        lines = result.stdout.strip().split('\n')
        assert len(lines) == 3
        assert lines[0] == "1"  # 1 occurrence in line 1
        assert lines[1] == "0"  # 0 occurrences in line 2
        assert lines[2] == "2"  # 2 occurrences in line 3

    def test_contar_handles_empty_text(self):
        """contar handles empty text input."""
        repl = SafeREPL()
        result = repl.execute("""
resultado = contar("", "gato")
print(resultado['total'])
print(len(resultado['por_linha']))
""")

        assert result.success is True
        assert "0" in result.stdout

    def test_contar_handles_empty_term(self):
        """contar handles empty search term."""
        repl = SafeREPL()
        result = repl.execute("""
...
""")
    ...
    def test_contar_returns_dict_with_required_keys(self):
        """contar returns a dict with total and por_linha keys."""
        repl = SafeREPL()
        result = repl.execute("""
texto = "O gato."
resultado = contar(texto, "gato")
print(isinstance(resultado, dict))
print('total' in resultado)
print('por_linha' in resultado)
""")

        assert result.success is True
        assert result.stdout.count("True") == 3

    def test_contar_por_linha_is_dict(self):
        """contar por_linha is a dict mapping line numbers to counts."""
        repl = SafeREPL()
        result = repl.execute("""
texto = "O gato."
resultado = contar(texto, "gato")
print(isinstance(resultado['por_linha'], dict))
""")

        assert result.success is True
        assert "True" in result.stdout

    def test_contar_is_not_saved_as_user_variable(self):
        """contar function is not counted as a user variable."""
        repl = SafeREPL()
        repl.execute("x = 1")

        user_vars = [v.name for v in repl.list_variables()]
        assert 'contar' not in user_vars
        assert 'x' in user_vars


class TestHelperFunctionExtrairSecao:
    """Test the extrair_secao() helper function available in REPL namespace."""

    def test_extrair_secao_is_available_in_namespace(self):
        """extrair_secao function is available in the REPL namespace."""
        repl = SafeREPL()
        result = repl.execute("print(callable(extrair_secao))")

        assert result.success is True
        assert "True" in result.stdout

    def test_extrair_secao_extracts_single_section(self):
        """extrair_secao extracts a single section between markers."""
        repl = SafeREPL()
        result = repl.execute("""
texto = "Antes\\n## Inicio\\nConteudo da secao\\n## Fim\\nDepois"
resultado = extrair_secao(texto, "## Inicio", "## Fim")
print(len(resultado))
print(resultado[0]['conteudo'])
""")

        assert result.success is True
        assert "1" in result.stdout
        assert "Conteudo da secao" in result.stdout

    def test_extrair_secao_extracts_multiple_sections(self):
        """extrair_secao extracts multiple sections between markers."""
        repl = SafeREPL()
        result = repl.execute("""
texto = "[START]Secao 1[END] texto [START]Secao 2[END]"
resultado = extrair_secao(texto, "[START]", "[END]")
print(len(resultado))
print(resultado[0]['conteudo'])
print(resultado[1]['conteudo'])
""")

        assert result.success is True
        lines = result.stdout.strip().split('\n')
        assert lines[0] == "2"
        assert lines[1] == "Secao 1"
        assert lines[2] == "Secao 2"

    def test_extrair_secao_returns_empty_for_no_matches(self):
        """extrair_secao returns empty list when markers are not found."""
        repl = SafeREPL()
        result = repl.execute("""
texto = "Texto sem marcadores"
resultado = extrair_secao(texto, "[START]", "[END]")
print(len(resultado))
""")
    ...
    def test_extrair_secao_is_case_insensitive(self):
        """extrair_secao is case-insensitive for markers."""
        repl = SafeREPL()
        result = repl.execute("""
texto = "Antes\\n## INICIO\\nConteudo\\n## fim\\nDepois"
resultado = extrair_secao(texto, "## inicio", "## FIM")
...
""")
    ...
    def test_extrair_secao_returns_positions(self):
        """extrair_secao returns start and end positions."""
        repl = SafeREPL()
        result = repl.execute("""
texto = "[START]conteudo[END]"
resultado = extrair_secao(texto, "[START]", "[END]")
print(resultado[0]['posicao_inicio'])
print(resultado[0]['posicao_fim'])
""")

        assert result.success is True
        lines = result.stdout.strip().split('\n')
        assert lines[0] == "0"  # posicao_inicio is start of [START]
        assert lines[1] == "20"  # posicao_fim is end of [END]

    def test_extrair_secao_returns_line_numbers(self):
        """extrair_secao returns line numbers for start and end."""
        repl = SafeREPL()
        result = repl.execute("""
texto = "Linha 1\\n[START]\\nConteudo linha 3\\nConteudo linha 4\\n[END]\\nLinha 6"
resultado = extrair_secao(texto, "[START]", "[END]")
print(resultado[0]['linha_inicio'])
print(resultado[0]['linha_fim'])
""")
        # Text structure:
        # Line 1: "Linha 1"
        # Line 2: "[START]" - content starts right after this, so linha_inicio = 2
        # Line 3: "Conteudo linha 3"
        # Line 4: "Conteudo linha 4"
        # Line 5: "[END]" - linha_fim = 5
        # Line 6: "Linha 6"
        assert result.success is True
        lines = result.stdout.strip().split('\n')
        assert lines[0] == "2"  # linha_inicio (right after [START] marker)
        assert lines[1] == "5"  # linha_fim (at [END])

    def test_extrair_secao_handles_empty_text(self):
        """extrair_secao handles empty text input."""
        repl = SafeREPL()
        result = repl.execute("""
resultado = extrair_secao("", "[START]", "[END]")
print(len(resultado))
""")
    ...
    def test_extrair_secao_handles_empty_markers(self):
        """extrair_secao handles empty markers."""
        repl = SafeREPL()
        result = repl.execute("""
resultado1 = extrair_secao("", "[START]", "[END]")
resultado2 = extrair_secao("texto", "", "[END]")
resultado3 = extrair_secao("texto", "[START]", "")
...
""")
    ...
    def test_extrair_secao_returns_list_of_dicts(self):
        """extrair_secao returns a list of dicts with required keys."""
        repl = SafeREPL()
        result = repl.execute("""
texto = "[START]conteudo[END]"
resultado = extrair_secao(texto, "[START]", "[END]")
print(isinstance(resultado, list))
print(isinstance(resultado[0], dict))
print('conteudo' in resultado[0])
print('posicao_inicio' in resultado[0])
print('posicao_fim' in resultado[0])
print('linha_inicio' in resultado[0])
print('linha_fim' in resultado[0])
""")

        assert result.success is True
        assert result.stdout.count("True") == 7

    def test_extrair_secao_handles_missing_end_marker(self):
        """extrair_secao returns empty when end marker is missing."""
        repl = SafeREPL()
        result = repl.execute("""
texto = "[START]conteudo sem fim"
resultado = extrair_secao(texto, "[START]", "[END]")
print(len(resultado))
""")
    ...
    def test_extrair_secao_handles_missing_start_marker(self):
        """extrair_secao returns empty when start marker is missing."""
        repl = SafeREPL()
        result = repl.execute("""
texto = "conteudo sem inicio[END]"
resultado = extrair_secao(texto, "[START]", "[END]")
print(len(resultado))
""")
    ...
    def test_extrair_secao_is_not_saved_as_user_variable(self):
        """extrair_secao function is not counted as a user variable."""
        repl = SafeREPL()
        repl.execute("x = 1")

        user_vars = [v.name for v in repl.list_variables()]
        assert 'extrair_secao' not in user_vars
        assert 'x' in user_vars


class TestHelperFunctionResumirTamanho:
    """Test the resumir_tamanho() helper function available in REPL namespace."""

    def test_resumir_tamanho_is_available_in_namespace(self):
        """resumir_tamanho function is available in the REPL namespace."""
        repl = SafeREPL()
        result = repl.execute("print(callable(resumir_tamanho))")

        assert result.success is True
        assert "True" in result.stdout

    def test_resumir_tamanho_converts_bytes(self):
        """resumir_tamanho converts bytes to '0.0 B' to '1023.0 B' range."""
        repl = SafeREPL()
        result = repl.execute("""
print(resumir_tamanho(0))
print(resumir_tamanho(500))
print(resumir_tamanho(1023))
""")

        assert result.success is True
        lines = result.stdout.strip().split('\n')
        assert lines[0] == "0.0 B"
        assert lines[1] == "500.0 B"
        assert lines[2] == "1023.0 B"

    def test_resumir_tamanho_converts_kilobytes(self):
        """resumir_tamanho converts to KB for values >= 1024 bytes."""
        repl = SafeREPL()
        result = repl.execute("""
print(resumir_tamanho(1024))
print(resumir_tamanho(1536))
print(resumir_tamanho(1024 * 1023))
""")

        assert result.success is True
        lines = result.stdout.strip().split('\n')
        assert lines[0] == "1.0 KB"
        assert lines[1] == "1.5 KB"
        assert lines[2] == "1023.0 KB"

    def test_resumir_tamanho_converts_megabytes(self):
        """resumir_tamanho converts to MB for values >= 1 MB."""
        repl = SafeREPL()
        result = repl.execute("""
print(resumir_tamanho(1024 * 1024))
print(resumir_tamanho(int(1.5 * 1024 * 1024)))
print(resumir_tamanho(100 * 1024 * 1024))
""")

        assert result.success is True
        lines = result.stdout.strip().split('\n')
        assert lines[0] == "1.0 MB"
        assert lines[1] == "1.5 MB"
        assert lines[2] == "100.0 MB"

    def test_resumir_tamanho_converts_gigabytes(self):
        """resumir_tamanho converts to GB for values >= 1 GB."""
        repl = SafeREPL()
        result = repl.execute("""
print(resumir_tamanho(1024 * 1024 * 1024))
print(resumir_tamanho(int(2.5 * 1024 * 1024 * 1024)))
""")

        assert result.success is True
        lines = result.stdout.strip().split('\n')
        assert lines[0] == "1.0 GB"
        assert lines[1] == "2.5 GB"

    def test_resumir_tamanho_converts_terabytes(self):
        """resumir_tamanho converts to TB for values >= 1 TB."""
        repl = SafeREPL()
        result = repl.execute("""
print(resumir_tamanho(1024 * 1024 * 1024 * 1024))
print(resumir_tamanho(int(1.5 * 1024 * 1024 * 1024 * 1024)))
""")

        assert result.success is True
        lines = result.stdout.strip().split('\n')
        assert lines[0] == "1.0 TB"
        assert lines[1] == "1.5 TB"

    def test_resumir_tamanho_handles_float_input(self):
        """resumir_tamanho handles float input values."""
        repl = SafeREPL()
        result = repl.execute("""
print(resumir_tamanho(1024.5))
print(resumir_tamanho(1536.75))
""")

        assert result.success is True
        lines = result.stdout.strip().split('\n')
        assert "1.0 KB" in lines[0]  # Close to 1 KB
        assert "1.5 KB" in lines[1]  # Close to 1.5 KB

    def test_resumir_tamanho_handles_negative_values(self):
        """resumir_tamanho handles negative values gracefully."""
        repl = SafeREPL()
        result = repl.execute("""
print(resumir_tamanho(-100))
""")

        assert result.success is True
        assert "valor negativo" in result.stdout

    def test_resumir_tamanho_handles_invalid_type(self):
        """resumir_tamanho handles invalid type gracefully."""
        repl = SafeREPL()
        result = repl.execute("""
print(resumir_tamanho("abc"))
print(resumir_tamanho([1, 2, 3]))
""")

        assert result.success is True
        assert "valor inválido" in result.stdout
        assert "str" in result.stdout
        assert "list" in result.stdout

    def test_resumir_tamanho_handles_zero(self):
        """resumir_tamanho handles zero correctly."""
        repl = SafeREPL()
        result = repl.execute("""
print(resumir_tamanho(0))
""")
    ...
    def test_resumir_tamanho_is_not_saved_as_user_variable(self):
        """resumir_tamanho function is not counted as a user variable."""
        repl = SafeREPL()
        repl.execute("x = 1")

        user_vars = [v.name for v in repl.list_variables()]
        assert 'resumir_tamanho' not in user_vars
        assert 'x' in user_vars
