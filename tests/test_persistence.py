...
class TestWALMode:
    ...
    def test_wal_mode_is_enabled(self, temp_db):
        ...
        # Query the database directly to check journal_mode
    ...
    def test_wal_mode_persists_after_operations(self, temp_db):
        """Test that WAL mode remains active after database operations."""
        pm = PersistenceManager(db_path=temp_db)

        # Perform some operations
        pm.save_variable("test_var", "test_value")
        pm.save_index("test_var", {"term": [0, 10]})
        pm.create_collection("test_collection")
        pm.add_to_collection("test_collection", ["test_var"])

        # Check WAL mode is still active
        with sqlite3.connect(temp_db) as conn:
            cursor = conn.cursor()
            cursor.execute("PRAGMA journal_mode")
            result = cursor.fetchone()
    ...
    def test_wal_files_created(self, temp_db):
        ...
        pm.save_variable("trigger_wal", "data to write")

        # WAL and SHM files should exist (or be created on first write)
        wal_file = temp_db + "-wal"
        shm_file = temp_db + "-shm"

        # Note: Files may not exist immediately on all systems if no writes occurred
        # But after a write operation, they should exist
        # The main assertion is that WAL mode is enabled (tested above)
        # This test verifies the side effect of WAL mode
        if os.path.exists(wal_file):
            assert os.path.isfile(wal_file)
        if os.path.exists(shm_file):
            assert os.path.isfile(shm_file)
...
class TestSQLitePerformanceOptimizations:
    """Tests for SQLite performance optimizations (WAL, synchronous, cache_size).

    These tests verify that the performance optimizations are correctly applied
    and compare performance between optimized and non-optimized settings.

    Note: WAL mode (journal_mode) persists in the database file, but synchronous
    and cache_size are per-connection settings that need to be set on each new
    connection. The PersistenceManager sets these in _init_db() but they apply
    only to that initialization connection.
    """

    def test_wal_mode_enabled_and_persists(self, temp_db):
        """Verify WAL mode is enabled and persists across connections.

        WAL mode is a database-level setting that persists in the file.
        """
        pm = PersistenceManager(db_path=temp_db)

        # WAL mode should persist even on new connections
        with sqlite3.connect(temp_db) as conn:
            cursor = conn.cursor()
            cursor.execute("PRAGMA journal_mode")
            result = cursor.fetchone()
    ...
    def test_synchronous_normal_is_configured(self, temp_db):
        """Verify synchronous=NORMAL is in the source code.

        Note: synchronous is a per-connection setting that doesn't persist.
        This test verifies it's configured in the _init_db method.
        """
        import inspect
        from rlm_mcp.persistence import PersistenceManager

        # Get the source code of _init_db
        source = inspect.getsource(PersistenceManager._init_db)

        # Verify the PRAGMA is present in the code
        assert "PRAGMA synchronous=NORMAL" in source, (
            "Expected PRAGMA synchronous=NORMAL in _init_db"
        )

    def test_cache_size_is_configured(self, temp_db):
        """Verify cache_size is configured in the source code.

        Note: cache_size is a per-connection setting that doesn't persist.
        This test verifies it's configured in the _init_db method.
        """
        import inspect
        from rlm_mcp.persistence import PersistenceManager
        ...
        # Verify the PRAGMA is present in the code
        assert "PRAGMA cache_size=-64000" in source, (
            "Expected PRAGMA cache_size=-64000 in _init_db"
        )

    def test_performance_comparison_batch_inserts(self, temp_db):
        """Compare insert performance between optimized and non-optimized SQLite.

        This test creates two databases: one with default settings and one with
        optimized settings (WAL, synchronous=NORMAL, cache_size=64MB).

        We measure the time to insert 100 variables and verify optimized is faster.
        """
        import time
        import tempfile

        # Create non-optimized database manually
        fd, non_optimized_db = tempfile.mkstemp(suffix=".db")
        os.close(fd)

        try:
            # Test data: 100 variables with moderate size
            test_data = [
                (f"var_{i:03d}", f"value_{'x' * 1000}_{i}")
                for i in range(100)
            ]

            # --- Non-optimized database (default settings) ---
            with sqlite3.connect(non_optimized_db) as conn:
                cursor = conn.cursor()
                # Use DELETE journal mode (default, slower than WAL)
                cursor.execute("PRAGMA journal_mode=DELETE")
                # Use FULL synchronous (default, more durable but slower)
            ...
            start_non_optimized = time.perf_counter()
            with sqlite3.connect(non_optimized_db) as conn:
                cursor = conn.cursor()
                for name, value in test_data:
                    cursor.execute(
                        "INSERT OR REPLACE INTO variables (name, data) VALUES (?, ?)",
                        (name, value)
                    )
                conn.commit()
            time_non_optimized = time.perf_counter() - start_non_optimized

            # --- Optimized database (WAL, synchronous=NORMAL, cache_size=64MB) ---
            with sqlite3.connect(temp_db) as conn:
                cursor = conn.cursor()
            ...
            print(f"  Non-optimized: {time_non_optimized:.4f}s")
            print(f"  Optimized:     {time_optimized:.4f}s")
            print(f"  Speedup:       {time_non_optimized / time_optimized:.2f}x")

            # Optimized should be faster (or at least not significantly slower)
            # We allow some margin because test environments can be unpredictable
            assert time_optimized <= time_non_optimized * 1.5, (
                f"Optimized ({time_optimized:.4f}s) should be faster than "
                f"non-optimized ({time_non_optimized:.4f}s)"
            )

        finally:
            if os.path.exists(non_optimized_db):
                os.unlink(non_optimized_db)
            # Clean up WAL files
            for ext in ["-wal", "-shm"]:
                wal_file = non_optimized_db + ext
                if os.path.exists(wal_file):
                    os.unlink(wal_file)

    def test_performance_comparison_batch_reads(self, temp_db):
        """Compare read performance between optimized and non-optimized SQLite.

        This test measures the time to read 100 variables repeatedly.
        """
        import time
        import tempfile
        ...
        try:
            ...
            # Read all variables 10 times (non-optimized)
            start_non_optimized = time.perf_counter()
            with sqlite3.connect(non_optimized_db) as conn:
                cursor = conn.cursor()
                for _ in range(10):
                    for name, _ in test_data:
                        cursor.execute(
                            "SELECT data FROM variables WHERE name = ?",
                            (name,)
                        )
                        cursor.fetchone()
            time_non_optimized = time.perf_counter() - start_non_optimized

            # Read all variables 10 times (optimized)
            start_optimized = time.perf_counter()
            with sqlite3.connect(temp_db) as conn:
                cursor = conn.cursor()
                for _ in range(10):
            ...
            print(f"\nPerformance comparison (1000 reads):")
            print(f"  Non-optimized: {time_non_optimized:.4f}s")
            print(f"  Optimized:     {time_optimized:.4f}s")
            print(f"  Speedup:       {time_non_optimized / time_optimized:.2f}x" if time_optimized > 0 else "N/A")

            # For reads, cache_size has the most impact
            # We allow some margin because test environments can vary
            assert time_optimized <= time_non_optimized * 2.0, (
                f"Optimized ({time_optimized:.4f}s) should not be significantly slower than "
                f"non-optimized ({time_non_optimized:.4f}s)"
            )
    ...
    def test_persistence_manager_wal_mode_persists(self, temp_db):
        """Verify PersistenceManager's WAL mode persists across connections.

        WAL (Write-Ahead Logging) is the only setting that persists in the
        database file. synchronous and cache_size are per-connection settings.
        """
        pm = PersistenceManager(db_path=temp_db)
    ...
    def test_all_optimizations_in_init_db(self, temp_db):
        """Verify all three SQLite optimizations are in the _init_db method."""
        import inspect
        from rlm_mcp.persistence import PersistenceManager

        source = inspect.getsource(PersistenceManager._init_db)

        # All three PRAGMAs should be present
        assert "PRAGMA journal_mode=WAL" in source, "Missing WAL mode"
        assert "PRAGMA synchronous=NORMAL" in source, "Missing synchronous=NORMAL"
        assert "PRAGMA cache_size=-64000" in source, "Missing cache_size"
